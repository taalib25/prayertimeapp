...
export const import2025PrayerTimes = async () => {
  try {
    const prayerTimesData2025: Omit<
      PrayerTimesData,
      'isInterpolated' | 'originalDate'
    >[] = [
      {
        date: '2025-01-01',
        day: 'Wednesday',
    ...
    const result = await bulkImportPrayerTimes(prayerTimesData2025);
    console.log(
      `✅ Successfully imported complete 2025 prayer times: ${result.imported} records`,
    );
    return result;
  } catch (error) {
